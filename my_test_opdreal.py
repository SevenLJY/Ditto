...
def test(model_id):
    ...
    exp_dir = os.path.join('test_results', 'opd_' + model_id)
    os.makedirs(exp_dir, exist_ok=True)
    # load npz data
    data = np.load(os.path.join(data_root, 'data/opdreal', model_id, 'test_ditto.npz'))
    ...
    gt_static_path = os.path.join(data_root, 'data/opdreal', os.path.dirname(data['start_path'].item())[11:], 'start_static_rotate.ply')
    gt_mobile_path = os.path.join(data_root, 'data/opdreal', os.path.dirname(data['start_path'].item())[11:], 'start_dynamic_rotate.ply')
    gt_whole_path = os.path.join(data_root, 'data/opdreal', data['start_path'].item()[11:])
    ...
    cd_s, cd_d, cd_w = eval_CD_ditto(pred_static_path, pred_mobile_path, pred_whole_path, gt_static_path, gt_mobile_path, gt_whole_path) # modify later
